<nav x-data="{ open: false }" class="bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700">
    ...
    <a href="{{ url('/') }}">
        <img src="{{ asset('images/logo.png') }}" alt="Anime & Manga Library Logo" class="h-9 w-auto">
    </a>
</div>

                <div class="hidden space-x-8 sm:ml-6 sm:flex">
                    <x-nav-link :href="route('mangas.index')" :active="request()->routeIs('mangas.*')">Mangas</x-nav-link>
                    <x-nav-link :href="route('animes.index')" :active="request()->routeIs('animes.*')">Animes</x-nav-link>

                    @auth
                        @if(auth()->user()->role === 'admin')
                            <x-nav-link :href="route('dashboard')">Admin</x-nav-link>
                        @endif
                    @endauth
                </div>
            </div>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            <x-responsive-nav-link :href="route('mangas.index')" :active="request()->routeIs('mangas.*')">Mangas</x-responsive-nav-link>
            <x-responsive-nav-link :href="route('animes.index')" :active="request()->routeIs('animes.*')">Animes</x-responsive-nav-link>
